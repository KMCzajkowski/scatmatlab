...
function result = ReadYaml(filename, nosuchfileaction, makeords, treatasdata, dictionary)
    ...
    ry = refractiveindex.YAMLMatlab.ReadYamlRaw(filename, 0, nosuchfileaction, treatasdata);
    ry = refractiveindex.YAMLMatlab.deflateimports(ry);
    if iscell(ry) && ...
        length(ry) == 1 && ...
        isstruct(ry{1}) && ...
    ...
    ry = refractiveindex.YAMLMatlab.mergeimports(ry);    
    ry = refractiveindex.YAMLMatlab.doinheritance(ry);
    ry = refractiveindex.YAMLMatlab.makematrices(ry, makeords);    
    if exist('dictionary','var')
        ry = refractiveindex.YAMLMatlab.dosubstitution(ry, dictionary);
    end;
