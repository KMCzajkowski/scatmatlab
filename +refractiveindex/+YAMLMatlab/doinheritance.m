...
function result = recurse(data, level, addit)
    if iscell(data) && ~refractiveindex.YAMLMatlab.ismymatrix(data)
...
function result = iter_cell(data, level, addit)
    ...
    for i = 1:length(data)
        if isstruct(result{i}) && isfield(result{i}, refractiveindex.YAMLMatlab.kwd_parent())
            result{i} = inherit(result{i}, result{i}.(refractiveindex.YAMLMatlab.kwd_parent()), [], addit{1}, {}); % !!!
        end;
    end;
end
...
function result = iter_struct(data, level, addit)
    ...
    for i = fields(result)'
        fld = char(i);
        if isstruct(result.(fld)) && isfield(result.(fld), refractiveindex.YAMLMatlab.kwd_parent())
            result.(fld) = inherit(result.(fld), result.(fld).(refractiveindex.YAMLMatlab.kwd_parent()), [], addit{1}, {});
        end;
    end;
end
